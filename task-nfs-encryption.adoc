...
= Create encrypted NFS sync relationships with NetApp Copy and Sync
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
. After the initial copy, Copy and Sync syncs any changed data every 24 hours. If there is data to sync, the process starts with the initiator opening an encrypted connection with the other data broker.
+
