...
+
A Red Hat Enterprise Linux system must be registered with Red Hat Subscription Management. If it is not registered, the system cannot access repositories to update required third-party software during installation.
* *RAM*: 16 GB
* *CPU*: 4 cores
...
. On the *Define Sync Relationship* page, choose a source and target then select *Continue*.
+
Complete the steps until you reach the *Data Broker Group* page.

. On the *Data Broker Group* page, select *Create Data Broker* then select *On-Prem Data Broker*.
+
...
. Enter a name for the data broker then select *Continue*.
+
After the instructions page loads, follow the instructions which include a unique link to download the installer. 

. On the instructions page:
+
.. Choose to enable access to *AWS*, *Google Cloud*, or both.
