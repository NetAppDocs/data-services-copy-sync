...
Copy and Sync starts syncing data between the source and target. Sync statistics on how long the sync took, whether it halted, and how many files were copied, scanned, or deleted are available. You can then manage your link:task-managing-relationships.html[sync relationships], link:task-managing-data-brokers.html[manage your data brokers], or link:task-managing-reports.html[create reports to optimize your performance and configuration].
